...
GROUP BY SUBJECT.ID, MARK.ID HAVING AVG(MARK.MARK) >(SELECT AVG(MARK) FROM MARK);
SELECT * FROM STUDENT
                  INNER JOIN PAYMENT ON  STUDENT.ID = PAYMENT.STUDENT_ID
GROUP BY STUDENT.ID, PAYMENT.ID HAVING AVG(PAYMENT.AMOUNT) >(SELECT AVG(AMOUNT) FROM PAYMENT);
